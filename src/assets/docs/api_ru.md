...
Доступные методы `@ngx-eco/ngx-themes`

---

## Использование

Модуль `@ngx-eco/ngx-themes` содержит сервис `NgxThemesService`, который предоставлят методы для манипуляции темами. Вот пример как вы можете использовать сервис в любом компоненте:
